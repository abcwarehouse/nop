...
SELECT COUNT(*) as DuplicateCount, PictureBinary, MAX(pic.Id) as PictureIdToSave, MAX(p.Id) as ProductId, MAX(p.Name) as ProductName
FROM [Picture] pic
JOIN Product_Picture_Mapping ppm ON pic.Id = ppm.PictureId 
JOIN Product p ON p.Id = ppm.ProductId and p.Published = 1
GROUP BY PictureBinary, ProductId HAVING COUNT(*) > 1
...
SELECT PictureBinary, pic.Id as PictureId, p.Id as ProductId, p.Name as ProductName
FROM [Picture] pic
	JOIN Product_Picture_Mapping ppm ON pic.Id = ppm.PictureId 
	JOIN Product p ON p.Id = ppm.ProductId and p.Published = 1 and p.Id IN (SELECT productId FROM @duplicateImages)
WHERE PictureBinary IN (SELECT pictureBinary FROM @duplicateImages)
	AND PictureId NOT IN (SELECT pictureIdToSave FROM @duplicateImages)
