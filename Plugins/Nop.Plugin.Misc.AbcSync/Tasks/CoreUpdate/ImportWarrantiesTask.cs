    ...
    class ImportWarrantiesTask : IScheduleTask
    {
        ...
        public void Execute()
        {
			...
			var command = $@"
				...
				WHEN NOT MATCHED BY TARGET
					THEN INSERT(ProductAttributeId, [Name], PriceAdjustment, WeightAdjustment, Cost, IsPreSelected, DisplayOrder) VALUES (S.ProdAttrId, S.[Name], S.[PriceAdjustment],0,0,0,1);
				--OUTPUT $action, inserted.*, deleted.*;
