...
namespace Nop.Plugin.Misc.AbcCore.Areas.Admin.Controllers
{
    ...
    public class MissingImageProductController : BaseAdminController
    {
        ...
        [HttpPost]
        public virtual IActionResult List(MissingImageProductSearchModel searchModel)
        {
            var products = _customProductService.GetProductsWithoutImages().ToPagedList(searchModel);
            var model = new MissingImageProductListModel().PrepareToGrid(searchModel, products, () =>
            {
