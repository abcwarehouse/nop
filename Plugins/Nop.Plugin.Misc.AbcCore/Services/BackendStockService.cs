...
namespace Nop.Plugin.Misc.AbcCore.Services
{
    public class BackendStockService : IBackendStockService
    {
        ...
        public Dictionary<int, int> GetStock(int productId)
        {
            ...
            string upc = _productService.GetProductById(productId).Sku;
        ...
        public StockResponse GetApiStock(int productId)
        {
            Product product = _productService.GetProductById(productId);
            if (product == null)
            {
            ...
            using (var client = new HttpClient())
            {
                ...
                catch (Exception exception)
                {
                    ...
                    _logger.InsertLog(Core.Domain.Logging.LogLevel.Information, "pickup api, error: " + exception.Message, internalException);
                    return null;
                }
            }
        }

        private StockResponse ReturnMockData()
        {
            _logger.Warning("BackendStockService: External call skipped, mocking response.");
            ...
            {
                Shop = _shopService.GetShopById(shopAbc.ShopId),
                ShopUrl = _urlRecordService.GetAllUrlRecords()
                                    .Where(u => u.EntityName == "Shop" && u.EntityId == shopAbc.ShopId)
                                    .Select(u => u.Slug).FirstOrDefault(),
                Available = true,
                Quantity = 1
