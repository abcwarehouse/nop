...
namespace Nop.Plugin.Misc.AbcCore.Extensions
{
    public static class ImportTaskExtensions
    {
        ...
        {
            EngineContext.Current.Resolve<ILogger>().Information($"Begin Task: {task.GetType().Name}");
        }

        public static void LogEnd(this IScheduleTask task)
        {
            EngineContext.Current.Resolve<ILogger>().Information($"End Task: {task.GetType().Name}");
        }

        public static void Skipped(this IScheduleTask task)
        {
            EngineContext.Current.Resolve<ILogger>().Warning($"Skipped Task: {task.GetType().Name}");
        }
        ...
        public static void DropIndexes()
        {
            try
            {
                EngineContext.Current.Resolve<INopDataProvider>().ExecuteNonQuery(GetSqlScript("Nopcommerce_Drop_IXs"));
            }
            catch (Exception ex)
            {
                EngineContext.Current.Resolve<ILogger>().Warning("Error when dropping indexes", ex);
            }
        }

        public static void CreateIndexes()
        {
            EngineContext.Current.Resolve<INopDataProvider>().ExecuteNonQuery(GetSqlScript("Nopcommerce_Create_IXs"));
        }
    }
}