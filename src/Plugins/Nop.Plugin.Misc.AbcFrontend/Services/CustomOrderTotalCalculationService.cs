...
namespace Nop.Plugin.Misc.AbcFrontend.Services
{
    public class CustomOrderTotalCalculationService : OrderTotalCalculationService
    {
        ...
        // Custom - calculates warranty tax
        public async override Task<(decimal discountAmount, List<Discount> appliedDiscounts, decimal subTotalWithoutDiscount, decimal subTotalWithDiscount, SortedDictionary<decimal, decimal> taxRates)> GetShoppingCartSubTotalAsync(
            ...
            bool includingTax)
        {
            ...
            foreach (var shoppingCartItem in cart)
            {
                ...
                var warrantyTaxResult = await _warrantyService.CalculateWarrantyTaxAsync(
                    shoppingCartItem, customer, sciExclTax
                );
                // , out taxRate, out sciInclTax
